...
public partial class AdminWindow : Window
{
    ...
    private static void ShowBannerAnimated(System.Windows.Controls.Border banner)
    {
        banner.Opacity = 0;
    ...
    private void MarkVersionDirty()
    {
        if (_isLoadingForm || _isRefreshing) return;
        _isVersionFormDirty = true;
        UpdateVersionFormHeader();
    }

    private void MarkPatchDirty()
    {
        if (_isLoadingForm || _isRefreshing) return;
        _isPatchFormDirty = true;
        UpdatePatchFormHeader();
    }

    private void UpdateVersionFormHeader()
    {
        var dirty = _isVersionFormDirty ? " *" : "";
        if (_isAddingNewVersion)
            txtVersionFormHeader.Text = $"Add New Version{dirty}";
        else if (lstVersions.SelectedItem is VersionDisplayItem item)
            txtVersionFormHeader.Text = $"Editing: {item.Id}{dirty}";
        else
            txtVersionFormHeader.Text = "Select a version to edit";
    }

    private void UpdatePatchFormHeader()
    {
        var dirty = _isPatchFormDirty ? " *" : "";
        if (_isAddingNewPatch)
            txtPatchFormHeader.Text = $"Add New Patch{dirty}";
        else if (lstPatches.SelectedItem is PatchDisplayItem item)
            txtPatchFormHeader.Text = $"Editing: {item.From}  ->  {item.To}{dirty}";
        else
            txtPatchFormHeader.Text = "Select a patch to edit";
    }

    private void TcAdminTabs_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (e.Source != tcAdminTabs || _isRefreshing || _handlingTabSwitch || !IsLoaded)
            return;

        if (_isVersionFormDirty || _isPatchFormDirty)
        {
            var which = _isVersionFormDirty ? "version" : "patch";
            if (!_dialogs.Confirm(
                    $"You have unsaved changes to the {which} form.\n\nDiscard changes and switch tabs?",
                    "Unsaved Changes",
                    useYesNo: true))
            {
                _handlingTabSwitch = true;
                tcAdminTabs.SelectedIndex = _previousTabIndex;
                _handlingTabSwitch = false;
                return;
            }

            // User accepted - discard and repopulate
            if (_isVersionFormDirty)
            {
                _isVersionFormDirty = false;
                PopulateVersionForm(lstVersions.SelectedItem as VersionDisplayItem);
            }
            if (_isPatchFormDirty)
            {
                _isPatchFormDirty = false;
                PopulatePatchForm(lstPatches.SelectedItem as PatchDisplayItem);
            }
        ...
        _previousTabIndex = tcAdminTabs.SelectedIndex;
    }

    private void TxtEditVersionName_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
    {
        MarkVersionDirty();
    }

    private void CmbEditPatchFrom_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        MarkPatchDirty();
    }

    private void CmbEditPatchTo_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        MarkPatchDirty();
    }
    ...
    private void TxtUpdateFeedPath_TextChanged(object sender, TextChangedEventArgs e)
    {
        if (bdAppUpdateStatus.Visibility == System.Windows.Visibility.Visible)
            bdAppUpdateStatus.Visibility = System.Windows.Visibility.Collapsed;
    }

    private async void BtnBrowseUpdateFeedPath_Click(object sender, RoutedEventArgs e)
    {
        var dialog = new OpenFolderDialog
        {
            Title = "Select app update feed folder",
            InitialDirectory = string.IsNullOrWhiteSpace(txtUpdateFeedPath.Text) ? Environment.CurrentDirectory : txtUpdateFeedPath.Text.Trim(),
            Multiselect = false
        };
        ...
        txtUpdateFeedPath.Text = dialog.FolderName;
        await SaveUpdateFeedPathAsync();
        ShowAppUpdateBanner(NotificationLevel.Info, "App update feed path saved.");
    }

    private async void BtnResetUpdateFeedPath_Click(object sender, RoutedEventArgs e)
    {
        txtUpdateFeedPath.Text = string.Empty;
        await SaveUpdateFeedPathAsync();
    ...
    private async void BtnCheckAppUpdates_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var feedPath = txtUpdateFeedPath.Text.Trim();
            var result = await _appUpdateService.CheckForUpdatesAsync(feedPath);
